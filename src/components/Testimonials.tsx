              ...
              <motion.div key={t.id} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }} className="t-card-inner">
                ...
                <div className="t-footer">
                  ...
                  <div className="t-img-wrap">
                    <img src={resolveUploadUrl(t.image)} alt={`${t.name} — ${t.category} testimonial`} loading="lazy" decoding="async" crossOrigin="anonymous" width={118} height={118} onError={(e) => { const t2 = e.target as HTMLImageElement; if (!t2.dataset.fallback) { t2.dataset.fallback='1'; t2.src=testimonials[0].image; } }} style={{ aspectRatio: '1 / 1' }} />
                  </div>
                </div>
              </motion.div>
            </AnimatePresence>
