      ...
      <div className="admin-ads-grid">
        {items.map((t, i) => (
          <motion.div key={t.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className={`admin-ad-card ${!t.active ? 'inactive' : ''}`}>
            <div className="admin-ad-card-image" style={{ height: 180 }}>{t.image ? <img onError={(e)=>{const t=e.target as HTMLImageElement; if(!t.dataset.fallback){t.dataset.fallback='1'; t.style.display='none';}}} width={400} height={400} loading="lazy" decoding="async" src={resolveUploadUrl(t.image)} alt={t.name} style={{ objectFit: 'cover' }} /> : <div className="admin-ad-card-placeholder">💬</div>}{!t.active && <div className="admin-ad-card-overlay">Inactive</div>}</div>
