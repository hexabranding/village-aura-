  ...
  return (
    <div className="admin-ads-page">
        ...
        <div className="admin-ads-grid">
          {heroSlides.map((item, i) => (
            <motion.div key={item.id || item._id || i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className={`admin-ad-card ${!item.active ? 'inactive' : ''}`}>
              {item.image ? <div className="admin-ad-card-image"><img onError={(e)=>{const t=e.target as HTMLImageElement; if(!t.dataset.fallback){t.dataset.fallback='1'; t.style.display='none';}}} width={400} height={400} loading="lazy" decoding="async" src={resolveUploadUrl(item.image)} alt={item.eyebrow} />{!item.active && <div className="admin-ad-card-overlay">Inactive</div>}</div> : <div className="admin-ad-card-placeholder">🏠</div>}
      ...
      <div className="admin-card" style={{ padding: '1.5rem', marginBottom: '1.5rem' }}>
        ...
        <div className="admin-form-grid">
          <div className="admin-form-group"><label>Image 1 (Left - Large)</label><div style={{ display: 'flex', gap: '0.5rem' }}><input value={weaver.image1} onChange={e => setWeaver(v => ({ ...v, image1: e.target.value }))} placeholder="Image URL or upload" style={{ flex: 1 }} /><input ref={fileRef1} type="file" accept="image/*" style={{ display: 'none' }} onChange={e => upload(e.target.files, url => setWeaver(v => ({ ...v, image1: url })), 'img1')} /><button type="button" onClick={() => fileRef1.current?.click()} style={{ padding: '0.5rem 1rem', border: '1px solid var(--maroon)', borderRadius: 'var(--radius-sm)', background: 'var(--ivory)', color: 'var(--maroon)', cursor: 'pointer' }}>{uploading === 'img1' ? '...' : 'Upload'}</button></div>{weaver.image1 && <img onError={(e)=>{const t=e.target as HTMLImageElement; if(!t.dataset.fallback){t.dataset.fallback='1'; t.style.display='none';}}} width={400} height={400} loading="lazy" decoding="async" src={resolveUploadUrl(weaver.image1)} alt="preview" style={{ width: '100%', maxHeight: 160, objectFit: 'cover', marginTop: '0.5rem', borderRadius: 8 }} />}</div>
          <div className="admin-form-group"><label>Image 2 (Right - Small)</label><div style={{ display: 'flex', gap: '0.5rem' }}><input value={weaver.image2} onChange={e => setWeaver(v => ({ ...v, image2: e.target.value }))} placeholder="Image URL or upload" style={{ flex: 1 }} /><input ref={fileRef2} type="file" accept="image/*" style={{ display: 'none' }} onChange={e => upload(e.target.files, url => setWeaver(v => ({ ...v, image2: url })), 'img2')} /><button type="button" onClick={() => fileRef2.current?.click()} style={{ padding: '0.5rem 1rem', border: '1px solid var(--maroon)', borderRadius: 'var(--radius-sm)', background: 'var(--ivory)', color: 'var(--maroon)', cursor: 'pointer' }}>{uploading === 'img2' ? '...' : 'Upload'}</button></div>{weaver.image2 && <img onError={(e)=>{const t=e.target as HTMLImageElement; if(!t.dataset.fallback){t.dataset.fallback='1'; t.style.display='none';}}} width={400} height={400} loading="lazy" decoding="async" src={resolveUploadUrl(weaver.image2)} alt="preview" style={{ width: '100%', maxHeight: 160, objectFit: 'cover', marginTop: '0.5rem', borderRadius: 8 }} />}</div>
        </div>
      ...
      <div className="admin-card" style={{ padding: '1.5rem', marginBottom: '1.5rem' }}>
        ...
        <div className="admin-ads-grid">
          {curated.map((item, i) => (
            <motion.div key={item.id || item._id || i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className={`admin-ad-card ${!item.active ? 'inactive' : ''}`}>
              {item.image ? <div className="admin-ad-card-image"><img onError={(e)=>{const t=e.target as HTMLImageElement; if(!t.dataset.fallback){t.dataset.fallback='1'; t.style.display='none';}}} width={400} height={400} loading="lazy" decoding="async" src={resolveUploadUrl(item.image)} alt={item.title} />{!item.active && <div className="admin-ad-card-overlay">Inactive</div>}</div> : <div className="admin-ad-card-placeholder">✨</div>}
              <div className="admin-ad-card-content">
      ...
      <div className="admin-card" style={{ padding: '1.5rem' }}>
        ...
        <div className="admin-ads-grid">
          {instagram.map((item, i) => (
            <motion.div key={item.id || item._id || i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className={`admin-ad-card ${!item.active ? 'inactive' : ''}`}>
              {item.image ? <div className="admin-ad-card-image"><img onError={(e)=>{const t=e.target as HTMLImageElement; if(!t.dataset.fallback){t.dataset.fallback='1'; t.style.display='none';}}} width={400} height={400} loading="lazy" decoding="async" src={resolveUploadUrl(item.image)} alt={item.label} />{!item.active && <div className="admin-ad-card-overlay">Inactive</div>}</div> : <div className="admin-ad-card-placeholder">📸</div>}
              <div className="admin-ad-card-content">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="admin-modal-overlay" onClick={() => setShowCuratedModal(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="admin-modal" onClick={e => e.stopPropagation()}>
              ...
              <div className="admin-modal-body">
                ...
                <div className="admin-form-group"><label>Image</label><div style={{ display: 'flex', gap: '0.5rem' }}><input value={form.image} onChange={e => setForm(f => ({ ...f, image: e.target.value }))} placeholder="Image URL or upload" style={{ flex: 1 }} /><input ref={fileRefCurated} type="file" accept="image/*" style={{ display: 'none' }} onChange={e => upload(e.target.files, url => setForm(f => ({ ...f, image: url })), 'curated')} /><button type="button" onClick={() => fileRefCurated.current?.click()} style={{ padding: '0.5rem 1rem', border: '1px solid var(--maroon)', background: 'var(--ivory)', color: 'var(--maroon)', borderRadius: 'var(--radius-sm)', cursor: 'pointer' }}>{uploading === 'curated' ? '...' : 'Upload'}</button></div>{form.image && <img onError={(e)=>{const t=e.target as HTMLImageElement; if(!t.dataset.fallback){t.dataset.fallback='1'; t.style.display='none';}}} width={400} height={400} loading="lazy" decoding="async" src={resolveUploadUrl(form.image)} alt="preview" style={{ width: '100%', maxHeight: 180, objectFit: 'cover', marginTop: '0.5rem', borderRadius: 8 }} />}</div>
                <div className="admin-form-grid">
