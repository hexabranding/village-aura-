                    ...
                    {order.items.map((item, idx) => {
                      ...
                      return (
                        <div key={idx} style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                          {product ? (
                            <img onError={(e)=>{const t=e.target as HTMLImageElement; if(!t.dataset.fallback){t.dataset.fallback='1'; t.style.display='none';}}} width={400} height={400} loading="lazy" decoding="async"
                                          ...
                                          <div style={{ background: 'white', borderRadius: 'var(--radius)', padding: '1.2rem', border: '1px solid #fecaca', boxShadow: '0 4px 20px rgba(239,68,68,0.08)' }}>
                                            ...
                                            <div style={{ marginTop:'0.75rem' }}>
                                              ...
                                              <div style={{ display:'flex', gap:'0.4rem', flexWrap:'wrap', marginTop:'0.4rem' }}>
                                                {returnImages.map((url,i)=> <div key={i} style={{ position:'relative', width:70, height:70, borderRadius:8, overflow:'hidden', border:'1px solid var(--line)' }}><img onError={(e)=>{const t=e.target as HTMLImageElement; if(!t.dataset.fallback){t.dataset.fallback='1'; t.style.display='none';}}} width={400} height={400} loading="lazy" decoding="async" src={resolveUploadUrl(url)} alt="evidence" style={{ width:'100%', height:'100%', objectFit:'cover' }} /><button onClick={()=>setReturnImages(prev=>prev.filter((_,idx)=>idx!==i))} style={{ position:'absolute', top:2, right:2, width:18, height:18, borderRadius:'50%', background:'#ef4444', color:'white', border:'none', fontSize:'0.6rem', cursor:'pointer' }}>✕</button></div>)}
