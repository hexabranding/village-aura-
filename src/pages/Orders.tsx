                                             ...
                                             return (
                                               <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                                                 <div style={{ display:'flex', gap:'0.4rem', flexWrap:'wrap', alignItems:'center' }}>
                                                  ...
                                                  {(ro.images?.length||0)>0 && <div style={{ display:'flex', gap:4, flexWrap:'wrap' }}>{ro.images!.slice(0,3).map((img:string,i:number)=><img key={i} src={img} alt="evidence" style={{ width:36, height:36, objectFit:'cover', borderRadius:6, border:'1px solid var(--line)' }} />)}{ro.video && <span style={{ fontSize:'0.6rem', background:'#000', color:'white', padding:'0.2rem 0.4rem', borderRadius:6 }}>▶ Video</span>}</div>}
                                          ...
                                          <div style={{ background: 'white', borderRadius: 'var(--radius)', padding: '1.2rem', border: '1px solid #fecaca', boxShadow: '0 4px 20px rgba(239,68,68,0.08)' }}>
                                            ...
                                            <div style={{ marginTop:'0.75rem' }}>
                                              ...
                                              <div style={{ display:'flex', gap:'0.4rem', flexWrap:'wrap', marginTop:'0.4rem' }}>
                                                {returnImages.map((url,i)=> <div key={i} style={{ position:'relative', width:70, height:70, borderRadius:8, overflow:'hidden', border:'1px solid var(--line)' }}><img src={url} alt="evidence" style={{ width:'100%', height:'100%', objectFit:'cover' }} /><button onClick={()=>setReturnImages(prev=>prev.filter((_,idx)=>idx!==i))} style={{ position:'absolute', top:2, right:2, width:18, height:18, borderRadius:'50%', background:'#ef4444', color:'white', border:'none', fontSize:'0.6rem', cursor:'pointer' }}>✕</button></div>)}
